import styles from './ChatMessageStyle'; // Style the component in a separate CSS file

const ChatMessage = ({ sender, message, time }) => {
  return (
    <>
    <div  style={sender === 'user' ? styles.userMessage  : styles.botMessage }>
    {sender != 'user' && <div style={{...styles.botMessageBefore, ...styles.botMessageBeforeLeft}}></div>}
    ...
    {sender === 'user' && <div style={{...styles.userMessageBeforeAfter, ...styles.userMessageAfter}}></div>}
    </div>
    </>
  );
};
